...
class ZalFragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        binding.container1.setOnClickListener {
            val action = ZalFragmentDirections.actionZalFragmentToNavigationHome(null,binding.locationName.toString())
            findNavController().navigate(action)
        }

        binding.container2.setOnClickListener {
            val action = ZalFragmentDirections.actionZalFragmentToNavigationHome(null,binding.locationName2.toString())
            findNavController().navigate(action)
        }
    }
